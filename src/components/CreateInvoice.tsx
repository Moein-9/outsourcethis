...
export const CreateInvoice: React.FC = () => {
  ...
  return (
    <div className="py-4">
      <h2 className="text-2xl font-bold mb-4">إنشاء فاتورة</h2>
      
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left Section: Main Form (spans 2 columns) */}
      ...
      <Sheet open={invoicePrintOpen} onOpenChange={setInvoicePrintOpen}>
        <SheetContent>
          ...
          <div className="py-4">
            ...
            <p>
              <strong>العميل:</strong> {currentPatient ? currentPatient.name : manualName} 
              ({currentPatient ? currentPatient.phone : manualPhone})
            </p>
            
            <hr className="my-3" />
            
            <p><strong>العدسة:</strong> {lensType} ({lensPrice.toFixed(2)} د.ك)</p>
            <p><strong>الطلاء:</strong> {coating || "بدون"} ({coatingPrice.toFixed(2)} د.ك)</p>
            <p>
              <strong>الإطار:</strong> {
                skipFrame 
                  ? "(لا يوجد إطار)" 
                  : `Brand: ${selectedFrame.brand}, Model: ${selectedFrame.model}, Color: ${selectedFrame.color}, Size: ${selectedFrame.size}, Price: ${selectedFrame.price.toFixed(2)} KWD`
              }
            </p>
            
            <hr className="my-3" />
            
            <p><strong>الخصم:</strong> {discount.toFixed(2)} د.ك</p>
            <p><strong>الإجمالي:</strong> {total.toFixed(2)} د.ك</p>
            <p><strong>الدفعة:</strong> {deposit.toFixed(2)} د.ك</p>
            <p><strong>المتبقي:</strong> {remaining.toFixed(2)} د.ك</p>
      ...
      <Sheet open={workOrderPrintOpen} onOpenChange={setWorkOrderPrintOpen}>
        <SheetContent>
          ...
          <div className="py-4">
            <p><strong>التاريخ والوقت:</strong> {new Date().toLocaleString("ar-EG")}</p>
            <p>
              <strong>اسم العميل:</strong> {currentPatient ? currentPatient.name : manualName} 
              ({currentPatient ? currentPatient.phone : manualPhone})
            </p>
            
            <hr className="my-3" />
            
            {currentPatient && currentPatient.rx && (
              <div className="my-4">
                <p className="font-bold mb-2">تفاصيل الوصفة:</p>
                <table className="w-full border-collapse ltr">
                  <thead>
                    <tr className="bg-muted">
                      <th className="p-1 border">العين</th>
            ...
            <p className="mt-3"><strong>نوع العدسة:</strong> {lensType}</p>
            <p><strong>الطلاء:</strong> {coating || "بدون"}</p>
            
            {!skipFrame && selectedFrame.brand && (
              <div className="mt-2">
                <p><strong>تفاصيل الإطار:</strong></p>
                <p>Brand: {selectedFrame.brand}</p>
                <p>Model: {selectedFrame.model}</p>
                <p>Color: {selectedFrame.color}</p>
                <p>Size: {selectedFrame.size}</p>
              </div>
            )}
