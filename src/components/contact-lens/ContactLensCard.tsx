...
export const ContactLensCard: React.FC<ContactLensCardProps> = ({
  ...
  const textColorClass = brandColorClass.includes("blue")
    ? "text-blue-600"
    : brandColorClass.includes("purple")
    ? "text-purple-600"
    : brandColorClass.includes("teal")
    ? "text-teal-600"
    : brandColorClass.includes("amber")
    ? "text-amber-600"
    : brandColorClass.includes("pink")
    ? "text-pink-600"
    : brandColorClass.includes("indigo")
    ? "text-indigo-600"
    : brandColorClass.includes("emerald")
    ? "text-emerald-600"
    : brandColorClass.includes("rose")
    ? "text-rose-600"
    : brandColorClass.includes("cyan")
    ? "text-cyan-600"
    : "text-orange-600";
  ...
  return (
    <Card
      className={`overflow-hidden hover:shadow-md transition-all duration-200 border bg-gradient-to-r ${brandColorClass}`}
    >
            ...
            <CardDescription className="mt-1 flex items-center gap-1">
              ...
              {lens.color && (
                <span className={`text-xs ${textColorClass}`}>
                  | {formatColorText(lens.color)}
                </span>
              )}
            </CardDescription>
          </div>
          <Badge
            variant={lens.qty > 5 ? "outline" : "destructive"}
            className={`text-xs ${lens.qty > 5 ? textColorClass : ""}`}
          >
