...
export const LensSelector: React.FC<LensSelectorProps> = ({
  ...
  const { lensCoatings, lensThicknesses, getLensCoatingsByCategory, getLensThicknessesByCategory, getLensPriceByCombination } = useInventoryStore();
  ...
  const handleCoatingSelect = (coating: LensCoating) => {
    ...
    // Reset thickness when coating changes
    setSelectedThickness(null);
    onSelectThickness(null);
  };
  ...
  // Get combined price if available
  const getCombinedPrice = (thickness: LensThickness): number => {
    if (selectedLensType && selectedCoating) {
      const combinedPrice = getLensPriceByCombination(
        selectedLensType.id,
        selectedCoating.id,
        thickness.id
      );
    ...
    // Return thickness price as fallback (shouldn't happen with proper setup)
    return thickness.price;
  };
        ...
        <Tabs defaultValue="lensType" className="w-full">
          <TabsList className="w-full mb-4 grid grid-cols-3">
            <TabsTrigger value="lensType" className="flex-1 bg-[#8B5CF6] data-[state=active]:bg-[#8B5CF6] data-[state=active]:text-white">
              <span className="text-white">1</span> - {language === 'ar' ? "اختر نوع العدسة" : t('selectLensType')}
            </TabsTrigger>
            <TabsTrigger value="coating" className="flex-1 bg-[#F97316] data-[state=active]:bg-[#F97316] data-[state=active]:text-white">
              <span className="text-white">2</span> - {language === 'ar' ? "اختر الطلاءات" : t('selectCoatings')}
            </TabsTrigger>
            ...
            {selectedLensType && selectedCoating ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {availableThicknesses.map((thickness) => {
                  const price = getCombinedPrice(thickness);
                  return (
                    <div
                      key={thickness.id}
                      className={`border rounded-lg p-3 cursor-pointer transition-all ${
              ...
              <div className="p-4 border border-dashed rounded-lg bg-muted/10 text-center">
                <p className="text-muted-foreground">
                  {selectedLensType 
                    ? (language === 'ar' ? "الرجاء اختيار الطلاء أولاً" : "Please select a coating first")
                    : (t('selectLensTypeFirst') || "Please select a lens type first")}
                </p>
              </div>
            )}
          </TabsContent>
