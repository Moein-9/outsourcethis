...
export const PrintService = {
  ...
  /**
   * Print an HTML element directly
   * @param element The HTML element or content to print
   * @param options Printing options
   * @param onComplete Callback after printing
   */
  printElement: (
    element: HTMLElement | string,
    options?: { pageTitle?: string; printBackground?: boolean },
    onComplete?: () => void
  ) => {
    try {
      ...
      // Prepare a basic document with the element content
      const docHtml = `
        <!DOCTYPE html>
        <html>
        <head>
          <title>${options?.pageTitle || 'Print'}</title>
          <meta charset="UTF-8">
      ...
      PrintService.printHtml(docHtml, 'receipt', onComplete);
    } catch (error) {
      console.error('[PrintService] Error in printElement:', error);
      if (onComplete) onComplete();
    }
  },
