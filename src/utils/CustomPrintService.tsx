    ...
    try {
          ...
          <head>
            ...
            <style>
              @media print {
                ...
                /* Make sure the RX table headers are always in English */
                table th {
      ...
      if (workOrderElement) {
        ...
        if (contentContainer) {
          ...
          // Update the work order ID to make sure it's showing the workOrderId, not the invoiceId
          if (workOrder?.id || invoice?.workOrderId) {
            const workOrderId = workOrder?.id || invoice?.workOrderId;
            const orderElements = printWindow.document.querySelectorAll('p:nth-child(2)');
            
            orderElements.forEach(element => {
              if (element instanceof HTMLElement && element.textContent === invoice?.invoiceId) {
                element.textContent = workOrderId;
              }
            });
          }
    ...
    try {
      ...
      if (invoiceElement) {
        ...
        if (contentContainer) {
          ...
          // Make sure it shows the invoice ID, not the work order ID
          if (invoice?.invoiceId) {
            const invoiceElements = printWindow.document.querySelectorAll('span.font-semibold.text-sm');
            
            invoiceElements.forEach(element => {
              if (element instanceof HTMLElement && element.textContent?.includes(invoice.workOrderId)) {
                element.textContent = `#${invoice.invoiceId}`;
              }
            });
          }
